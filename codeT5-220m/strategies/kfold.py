...
def run(args):
    ...
    base_output_dir = os.path.join(constants.OUTPUT_DIRS['kfold'], args.model.replace("/", "_"))
    ...
    # KFold setup
    kfold = KFold(n_splits=args.k, shuffle=True, random_state=42)
    fold_results = []
    
    for fold, (train_idx, val_idx) in enumerate(kfold.split(texts)):
        print(f"\n{'='*40}")
        print(f"Training fold {fold+1}/{args.k}")
        print(f"{'='*40}")
