...
def training_test_validation(num_epochs, device):
	...
	tracker = EmissionsTracker(
		project_name="GraphCodeBERT_Standard_Training",
		output_dir=SAVE_FOLDER,
		output_file="emissions_standard.csv"
	)
	...
	data_loader = PIIDataLoader(train_texts=train_texts, train_labels=train_labels, valid_texts=valid_texts, valid_labels=valid_labels, tokenizer_name=MODEL_NAME, test_texts=test_texts, test_labels=test_labels)
	train_loader, valid_loader, test_loader = data_loader.get_dataloader()

	# Initialize BERT model
	model_trainer = GraphCodeBertModelTrainer(model_name=MODEL_NAME, save_folder=SAVE_FOLDER, device=device, num_epochs=num_epochs)
	...
	print(f"Total emissions for standard training: {emissions:.6f} kg CO2 equivalent")
...
def arg_commandline():
	...
	parser = argparse.ArgumentParser(description="GraphCodeBERT model training with different validation strategies")
